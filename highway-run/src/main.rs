    ...
    fn update(&mut self, engine: &mut Engine, acc: f32) {
        match self.game_state {
            ...
            GameState::InGame => {
                ...
                // // for jumping
                // if engine.input.is_key_pressed(engine::Key::Up) {
                //     println!("jump!");
                // }
                ...
                // play building collision sound
                // Inside the section where you handle building collisions
                // if let Some(idx) = self
                // .buildings
                // .iter()
                // .position(|building: &Sprite| building.pos.distance(self.bus.pos) <= COLLISION_DISTANCE)
                // {
                // self.buildings.swap_remove(idx);
                // self.score += 1;

                // // Play the building collecting sound
                // self.audio_state.audio_manager.play(&self.audio_state.building_collect_sound, Default::default())?;
                // }
            }
        }
    }
